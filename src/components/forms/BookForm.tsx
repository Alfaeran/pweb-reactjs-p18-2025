...
const BookForm: React.FC<BookFormProps> = ({ initialData, onSubmit, submitLabel, loading, submitError }) => {
  ...
  useEffect(() => {
    const fetchGenres = async () => {
      try {
        ...
        console.log('✅ Loaded genres from service:', genreData);
        setGenres(genreData || []);
        if (!genreData || genreData.length === 0) {
          setFormError('No genres available in database.');
        }
      } catch (err: any) {
        console.error('❌ Failed to fetch genres:', err);
        setFormError(`Failed to load genres: ${err.response?.data?.message || err.message}`);
      } finally {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
      ...
      <div style={{ display: 'flex', gap: '15px' }}>
        <div style={{ flex: 1 }}>
            <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', color: '#3b2a1a' }}>🎭 Genre</label>
            <select 
              name="genre_id" 
